...
export default defineNuxtPlugin((nuxtApp) => {
  ...
  const directusUrl = process.server ? config.public.API_URL : directusProxyUrl;
    ...
    customFetch = $fetch.create({
      onRequest({ request, options }) {
        ...
        if (
          ...
          request.startsWith(directusProxyUrl)
        ) {
  ...
  let directus = createDirectus<Schema>(directusUrl, {
  ...
  if (process.client) {
    const realtimeUrl = new URL(joinURL(config.public.API_URL, "websocket"));
