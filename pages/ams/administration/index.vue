...
const { deleteFile, uploadFiles } = useDirectusFiles()
...
async function saveAvatar() {
	await cropper.value.getCroppedCanvas().toBlob(async (blob: Blob) => {
		...
		try {
			...
			await updateUser(edit_user.value?.id, { avatar: new_avatar.id }, { fields: ['id'] })
...
const handleProfileEdit = (user: any) => {
	...
	edit_formdata.current_residence = landing_zones.value?.find((e: any) => e.id === userData.current_residence_value) || ''
	edit_formdata.birthdate = computed(() => `${edit_birthdate_year.value}-${edit_birthdate_month.value}-${edit_birthdate_day.value}`)
...
async function onProfileEditSubmit(event: FormSubmitEvent<EditSchema>) {
	...
	for (const key in edit_formdata) {
		if (edit_formdata[key as keyof typeof edit_formdata] !== initialEditFormdata[key as keyof typeof initialEditFormdata]) {
	...
	try {
		...
		for (const key in edit_formdata) {
			if (initialEditFormdata[key as keyof typeof initialEditFormdata] !== edit_formdata[key as keyof typeof edit_formdata]) {
				initialEditFormdata[key as keyof typeof initialEditFormdata] = edit_formdata[key as keyof typeof edit_formdata]
			}
		}
			...
			<div v-if="modalStore.type === 'userCreationSuccess'">
				...
				<table class="prose prose-invert">
					<tbody>
						...
						<tr
							v-if="modalStore.data?.last_name"
				...
				<h4 class="text-danger">
					...
					Sicherheitsgründen manuell erfolgen. Wähle ihn hierzu einfach in der Tabelle aus und klicke unter &quot;Optionen&quot; auf:
					&quot;Freischalten&quot;
				</h4>
		...
		<template #slideContent>
				...
				>
                  ...
                  um.
					...
					>
						...
						<template
							v-if="userFormData?.role?.id === 'bc712fc8-ce4f-4427-b431-4942eaaedaa6'"
							#help
						>
							<span
								class="text-danger"
			...
			>
				<div class="divide-y divide-bsecondary space-y-6 *:pt-6 first:*:pt-2 mb-6">
					...
					>
						<UInput
							v-model="edit_formdata.first_name"
							placeholder="Chris"
							size="md"
							autocomplete="off"
						/>
					...
					>
						<UInput
							...
							autocomplete="off"
							:model-value="
								useSlugify(edit_formdata.first_name.trim() + (edit_formdata.last_name ? ' ' + edit_formdata.last_name.trim() : ''), true)
							"
						...
						<UInput
							...
							autocomplete="off"
							:model-value="
								useSlugify(edit_formdata.first_name.trim() + (edit_formdata.last_name ? ' ' + edit_formdata.last_name.trim() : ''))
							"
					...
					<div class="flex flex-wrap items-center justify-between gap-4">
						<div class="flex items-start gap-4">
							<div>
								<p class="p-0 font-semibold dark:text-white">
					...
					>
						<div class="relative">
							...
							>
								...
								/>
								<template #label>
									<span v-if="edit_formdata.birthplace">{{ edit_formdata.birthplace.path_label || edit_formdata.birthplace.name }}</span>
									<span
										v-else
										class="text-[13.9px]"
									>Keine Landezone ausgewählt</span>
					...
					<div class="flex flex-wrap items-center justify-between gap-4">
						<div class="flex items-start gap-4">
							<div>
								...
								<p class="p-0 mt-1 text-sm text-tbase/50">
									Nun geht es um deinen Bürgerstatus.
								</p>
							</div>
						</div>
					</div>
