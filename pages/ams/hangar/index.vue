...
const schema = object({
  ...
  showName: string().required('Es ist eine Schiffsnamens-Sichtbarkeit erforderlich!'),
  module: object().nullable(),
  planned: string().required('Es ist ein Status erforderlich!'),
});
...
const initialFormData: IFormData = reactive({ ...formData });
  ...
  try {
    ...
    await updateItem({
      ...
      item: {
        ...
        publicName: event.data.showName,
...
console.log(data.value.shipList);
definePageMeta({
  layout: false,
  middleware: 'auth',
});
...
<template>
  <NuxtLayout name="ams">
    ...
    <template #slideContent>
      <UForm ref="form" :state="formData" :schema="schema" validateOn="submit" @submit="onEditSubmit">
                ...
                <template #panel>
                  <div class="p-1 text-xs text-left text-tbase/60 whitespace-break-spaces">
                    ...
                    <p>
                      ...
                      <NuxtLink target="_blank" to="https://www.fantasynamegenerators.com/ship-names.php"
                        >hier</NuxtLink
                      >
                      um.
                    </p>
          ...
          <UFormGroup size="xl" label="Modul" name="module" :ui="formgroupUi">
            ...
            <template #hint>
              <UPopover mode="hover">
                ...
                <template #panel>
                  <div class="p-1 text-xs text-left text-tbase/60 whitespace-break-spaces">
                    <p>Falls dein Schiff Modular ist, kannst du hier das aktuelle Modul deines Schiffes auswählen.</p>
                  </div>
                </template>
              </UPopover>
            </template>
              ...
              <UPopover mode="hover">
                ...
                <template #panel>
                  <div class="p-1 text-xs text-left text-tbase/60 whitespace-break-spaces">
                    ...
                    <p>
                      <span class="text-primary">Information</span>: Bitte wähle hier nur fest geplante Schiffe aus. Für
                      alles andere hast du deine private Wunschliste.
                    </p>
                  </div>
                </template>
              </UPopover>
    ...
    <template #slideFooter>
      <div class="flex flex-wrap justify-between w-full px-8">
        <ButtonDefault type="button" @click="modalStore.closeSlide" class="w-1/3" color="danger"
          >Schließen</ButtonDefault
        >
        <ButtonDefault
