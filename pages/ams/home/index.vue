    ...
    <div>
      ...
      >
        <NuxtLink
          ...
          :key="item.name"
          :to="user.permissionLevel >= item.level && item.released ? '/ams' + item.link : null"
        ...
        >
          <div
            ...
            :class="{ 'opacity-40': user.permissionLevel < item.level || !item.released }"
          ...
          <div class="absolute bottom-0 z-10 flex w-full h-full mt-1 transition-group">
            <div class="mx-auto mt-[100%] transition-group group-hover:mt-[45%]">
              ...
              <div class="hidden group-hover:contents">
                <p v-if="user.permissionLevel >= item.level && item.released" class="p-0 text-sm text-white">
                  {{ item.name }}
                </p>
                <p v-else-if="!item.released" class="p-0 text-white/80">Bald verfügbar...</p>
                <p
                  v-else-if="user.permissionLevel < item.level"
