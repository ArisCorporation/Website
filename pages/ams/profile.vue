...
const { directus, readMe, updateMe, uploadFiles, deleteFile } = useCMS();
...
async function saveAvatar() {
  ...
  await cropper.value.getCroppedCanvas().toBlob(async (blob: Blob) => {
    ...
    try {
      ...
      await directus.request(updateMe({ avatar: new_avatar.id }, { fields: ['id'] }));
        ...
        <div class="flex-1 overflow-hidden outline-none">
          ...
          <p class="text-[#444] items-center flex space-x-1 text-sm sm:text-base absolute bottom-4 p-0">
            ...
            <NuxtLink
              ...
              class="my-auto transition opacity-75 decoration-transparent hover:opacity-100 text-aris-400 animate-link"
            >
              Biografie
            </NuxtLink>
          </p>
