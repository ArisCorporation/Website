...
const { data: places } = await readAsyncItems('systems', {
	query: {
	...
	<div>
		<div class="flex flex-wrap justify-between">
			<div class="mt-auto">
				...
				<h4 class="mb-0 text-xs md:text-lg">
					<span class="text-dark-gray">Rollen: </span><span class="italic text-light-gray">{{ data.roles?.join(', ') || 'N/A' }}</span>
				</h4>
			</div>
			<div>
				<NuxtLink to="/verseexkurs/companies/ariscorp">
					<Icon
						...
						class="h-20 -mb-4 md:-mb-8 md:h-40 w-fit"
					/>
				</NuxtLink>
			</div>
		</div>
		...
		<div class="grid grid-cols-3 gap-4">
			<div class="col-span-3 space-y-4 xl:col-span-1">
				...
				<TableParent title="ArisCorp">
					...
					<TableRow
						full-width
						title="Position"
						:content="data.position.name"
					/>
					<TableRow
						full-width
						title="Rollen innerhalb der ArisCorp"
          ...
          /> -->
					...
					<TableRow
						title="Haarfarbe"
						:content="data.hair_color"
					/>
					<TableRow
						title="Augenfarbe"
						:content="data.eye_color"
					/>
					<TableHr />
					<TableRow
						title="Bürgerstatus"
					...
					<TableRow
						...
						:content="data.citizen_state && data.citizen_reason"
					/>
				</TableParent>
				<TableParent
					v-if="data.duty_state"
					title="Militärischer Dienst"
				>
					<TableRow
						title="Dienstzeit"
						:content="data.duty.period"
					/>
					<TableRow
						title="Division"
						:content="data.duty.division"
					/>
					<TableRow
						title="Entlassen"
						:content="data.duty.end"
					/>
				</TableParent>
				<TableParent
					v-if="data.education_state"
					title="Besondere Bildung"
				>
					<TableRow
						title="Name der Ausbildung"
						:content="data.education.name"
					/>
					<TableRow
						title="Ausbildungszeit"
						:content="data.education.period"
					/>
					<TableRow
						title="Hochschule"
						content="COMING SOON"
				...
				<TableParent title="Spezifisch">
					...
					/>
					<TableRow
						title="Rätselhafte Züge"
						:content="data.mysterious_things"
						is-list
					/>
					<TableRow
						title="Hervorstechender Charakterzug"
				...
				<TableParent title="Geschmäcker">
					...
					<TableHr />
					<TableRow
						title="Typische Kleidung"
						:content="data.clothing"
						is-list
					/>
					<TableRow
					...
					/>
					<TableRow
						title="Lieblingsalkohol"
						:content="data.alcohol"
						third
						is-list
					/>
					<TableHr />
					<TableRow
						:title="data.pronom + ' liebt'"
						:content="data.loves"
						is-list
					/>
					<TableRow
						:title="data.pronom + ' hast'"
						:content="data.hates"
						is-list
					/>
				</TableParent>
				...
				<template v-if="selectedTab === 0">
					...
					>
						<div class="px-8">
							...
							>
								<h1
									className="text-xl sm:text-2xl md:text-4xl redacted m-0"
									data-text="[ REDACTED ]"
								>
									[ REDACTED ]
								</h1>
								...
								>
									...
									>
										<h2 class="text-center text-secondary">
											Der Hangar von {{ data.full_name }} ist leer.
										</h2>
									</div>
								</Transition>
							</ClientOnly>
