...
const { readAsyncItem } = useDirectusItems()
const { params } = useRoute()

const { data } = await readAsyncItem('comm_links', String(params.id), {
	query: {
		fields: [
			'id',
			'title',
		...
		<div class="mb-3">
			...
			<DefaultPanel>
				<NuxtImg
					...
					class="object-cover w-full mx-auto max-h-96"
				/>
			</DefaultPanel>
			...
			<div class="flex flex-wrap justify-between space-y-4">
				...
				>
					...
					<p class="mt-auto ml-2 italic uppercase">
						<span class="text-secondary">Author: </span><NuxtLink
							class="text-tbase"
							:to="'/biography/' + data?.author?.slug"
						>{{
