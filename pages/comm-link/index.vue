...
const { data: commLinksCount } = await readAsyncItems('comm_links', {
	query: {
		fields: ['id'],
		filter: filters,
		limit: -1,
...
watch(commLinks, () => filteredCommLinks.value = commLinks.value.map((obj: any) => transformCommLink(obj, commLinks.value)))
...
<template>
	<div>
		...
		<div class="comm-link-grid">
			<CommLinkCard
				v-for="commLink in filteredCommLinks"
				:key="commLink.id"
				:data="commLink"
				:size="commLink.size"
			/>
