...
export const useSearch = (
  ...
  options?: {
    debounce?: boolean;
    query?: boolean;
    typingAction?: Function;
    debounceAction?: Function;
    query_name?: string;
    options?: (typeof search.value)[];
  },
): void => {
  ...
  if (typeof search.value !== 'string') {
    if (route.query[options?.query_name || 'q']) {
      search.value = options?.options?.find(
        (option: typeof search.value) => option?.id === route.query[options?.query_name || 'q'],
      );
    }
    ...
    watch(search, () => {
      ...
      if (!Array.isArray(search.value)) {
        replace({
          query: search.value?.id
            ? ({ ...route.query, [options?.query_name || 'q']: search.value?.id } as LocationQueryRaw)
            : { ...route.query, [options?.query_name || 'q']: undefined },
        });
      } else {
				console.log(search.value[0] ? true : false)
        replace({
          query: search.value[0]
            ? ({ ...route.query, [options?.query_name || 'q']: search.value } as LocationQueryRaw)
            : { ...route.query, [options?.query_name || 'q']: undefined },
        });
      }
    });
      ...
      if (options?.query === true) {
        replace({
          query: search_input.value
            ? ({ ...route.query, [options?.query_name || 'q']: search_input.value } as LocationQueryRaw)
            : { ...route.query, [options?.query_name || 'q']: undefined },
        });
      }
