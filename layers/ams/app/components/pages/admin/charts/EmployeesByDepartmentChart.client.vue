...
const membersByDepartment = computed(() =>
  processDonutChartData(props.users, (user) => {
    const departments: string[] = []
    if (user.primary_department?.name) {
      departments.push(user.primary_department.name)
    }
    if (user.secondary_department?.name) {
...
<template>
  <UCard variant="ams" class="w-full">
    ...
    <div class="flex h-full items-center px-8 space-x-6">
      <DonutChart
        :data="membersByDepartment.data"
        :labels="membersByDepartment.labels"
        :height="300"
        :radius="0"
        :arcWidth="40"
        :hideLegend="true"
        ...
        <template #tooltip="{ values }">
          <div
            ...
            :style="[
              {
                'border-color':
                  membersByDepartment.labels[values?.index]?.color,
              },
            ]"
          >
            ...
            <div class="col-span-4">
              {{ membersByDepartment.labels[values?.index]?.name }}
            </div>
          </div>
        </template>
      ...
      <div class="w-full">
        <ul class="divide-y divide-(--ui-bg-accented) text-sm">
          <li
            v-for="(label, index) in membersByDepartment.labels"
            :key="label.name"
          ...
          >
            ...
            <span>{{ membersByDepartment.data[index] }}</span>
          </li>
        </ul>
      </div>
    </div>
