...
const commercialShowOverlay = computed(
  () => commercialShowPlayingOverlay.value || commercialWaiting.value
)
...
async function toggleCommercialMute() {
  ...
  commercialOverlayIcon.value = commercialMuted.value
...
async function adjustCommercialVolume(amount: number) {
  commercialVolume.value = Math.max(
    0,
    Math.min(commercialVolume.value + amount, 1)
  )
  amount > 0
          ...
          >
            ...
            >
              <div
                ...
                class="absolute p-1.5 rounded-full bg-black/75 size-14"
              >
                <UIcon
                  :name="commercialOverlayIcon"
                  class="opacity-50 size-full"
                />
              </div>
            </Transition>
            <Transition
        ...
        <div class="w-full px-2 pb-2 space-y-2 my-auto">
          ...
          <div class="flex justify-between w-full h-full">
            <div class="flex items-center gap-x-2">
              ...
              />
              <div>
                {{ formatDuration(commercialCurrentTime) }} /
                {{ formatDuration(commercialDuration) }}
              </div>
            </div>
            <div class="flex items-center gap-x-2">
              <UButton
