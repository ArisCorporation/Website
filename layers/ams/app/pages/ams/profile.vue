...
async function handleFormSubmit(event: FormSubmitEvent<ProfileFormData>) {
  ...
  const success = await profileEdit.submitProfile(dataToSend)
  if (success) {
    useToast().add({
...
const validateFormWithApiErrors = (formState: ProfileFormData): FormError[] => {
...
<template>
  <div>
    ...
    <UForm
      ...
      :state="profileEdit.currentFormData"
    ...
    >
      <UAlert
        ...
        color="error"
        variant="outline"
        :title="profileEdit.getSubmitError"
        class="mb-4"
