export default defineAppConfig({
  ui: {
    ...
    table: {
      slots: {
        root: 'relative overflow-x-auto overflow-y-hidden before:pointer-events-none before:absolute before:inset-x-6 before:top-0 before:z-[2] before:h-px before:bg-[linear-gradient(90deg,transparent,rgba(0,255,232,0.55),transparent)]',
        ...
        tbody:
          'divide-y divide-(--ui-primary)/10 [&>tr:nth-child(odd)]:bg-white/[0.015] [&>tr]:data-[selectable=true]:hover:bg-[linear-gradient(90deg,rgba(0,255,232,0.07),rgba(255,255,255,0.025)_45%,transparent)] [&>tr]:data-[selectable=true]:focus-visible:outline-(--ui-primary)',
        tfoot:
          'bg-[linear-gradient(180deg,rgba(9,15,32,0.88)_0%,rgba(4,9,22,0.96)_100%)]',
        tr: 'group transition-[background-color,box-shadow] duration-200 data-[selected=true]:bg-(--ui-primary)/10 hover:shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]',
        ...
        td: 'px-4 py-3.5 text-sm text-(--ui-text) align-middle whitespace-nowrap first:ps-5 last:pe-5 [&:has([role=checkbox])]:pe-0 group-hover:text-white/95',
