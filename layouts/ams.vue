...
<template>
	<div class="lg:grid lg:grid-cols-[16rem,_1fr]">
		...
		<div class="flex flex-col justify-between flex-1 w-full max-w-full min-h-screen">
			<SidebarOverlay
				:state="SidebarStore.MobileSidebar"
				@click="SidebarStore.toggleMobileSidebar"
			/>

			<div
				v-if="JSON.parse(useCookie('devtools').value)"
				class="bg-black z-[99] pb-4 px-8"
			>
				<h6>DEV TOOLS:</h6>
			...
			<div class="flex flex-wrap w-full px-4 mt-4">
				<Icon
					name="IconsLogosAmsBanner"
					class="w-1/4 aspect-[33/11] -mb-2 h-auto"
				/>
				<div class="relative mt-auto ml-auto group">
					<div class="absolute right-0 justify-center hidden mt-2 text-center w-14 group-hover:flex">
						Hilfe
					</div>
				</div>
			</div>
