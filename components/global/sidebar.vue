	...
	>
		<div class="relative h-full pb-4 overflow-y-auto bg-bsecondary">
			<div class="flex w-full px-4">
				<Icon
					:name="banner"
					class="w-3/4 px-4 mx-auto -mt-0.5 -mb-0.5 h-auto"
				/>
			</div>
			...
			<ul class="absolute bottom-0 p-0 mt-auto space-y-2 font-medium list-none basis-full">
				<li
					v-for="(item, i) in userSidebarItems"
					:key="i"
					class="pb-0"
				>
					<!-- class="relative flex items-center p-2 mx-3 rounded-lg hover:no-underline group hover:bg-bprimary before:transition-default" -->
					...
					>
						<Icon
							:name="item.icon"
							class="w-6 h-6 transition-group text-tbase/75 group-hover:text-white"
						/>
						<span class="ms-3">{{ item.name }}</span>
					</button>
				</li>
				<li>
					...
					>
						<Icon
							...
							class="w-6 h-6 transition duration-200 text-tbase group-hover:text-white"
						/>
