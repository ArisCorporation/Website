...
const { directus, readUsers } = useCMS();
...
// TODO
const { data: baseItemCount } = await useAsyncData('AMS:ADMINISTRATION_TABLE_COUNT', () =>
  directus.request(
    readUsers({
      limit: -1,
      fields: ['id'],
        ...
        <div class="w-full divide-y divide-btertiary">
          <div class="flex flex-wrap items-center justify-between w-full px-4 py-4 lg:flex-nowrap gap-y-2">
            <div class="relative w-full lg:w-1/4">
              <UInput ref="search_input" v-model="search" size="md" placeholder="Vorname, Nachname, Abteilung, ..." />
              <button
                v-if="search !== ''"
                type="button"
                ...
                <template #avatar>
                  <UAvatarGroup size="xs" :max="8">
                    <UAvatar
                      ...
                      :src="$config.public.fileBase + (item.avatar ?? '8658f40d-77d9-44c4-8f0d-af820855a3bc')"
                      :size="32"
                    />
                    <NuxtLink
                    ...
                    >
                      <div class="flex items-center my-auto gap-x-2">
                        <UAvatar
                          ...
                          :src="
                            $config.public.fileBase + (selectedRows[0].avatar ?? '8658f40d-77d9-44c4-8f0d-af820855a3bc')
                          "
                          :size="32"
                        />
                    ...
                    <UAvatar
                      v-else
                      label="'None'"
                      :src="$config.public.fileBase + '8658f40d-77d9-44c4-8f0d-af820855a3bc'"
                      :size="32"
                    />
                  </UAvatarGroup>
                </template>
